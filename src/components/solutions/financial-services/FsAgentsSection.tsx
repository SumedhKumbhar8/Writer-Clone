...
export function FsAgentsSection() {
  const { agents } = fsPageData;
  ...
  return (
    <section className="ai-agent-marketing-wrapper bg-[#1C1B1E] py-[70px] pb-[135px] max-lg:py-[60px]">
      <div className="container-default-sm">
        ...
        <div className="scroller_list mb-12 inline-flex items-center rounded-[128px] border border-[#333] bg-black p-[13px_69px] shadow-[0_0_50px_0_rgba(60,100,250,0.10)] max-lg:w-full max-lg:overflow-x-auto max-lg:p-[13px_20px]">
          {agents.tabs.map((tab) => (
            <button
              key={tab.id}
              type="button"
        ...
        <div className="ap_tabs_wrap flex flex-col gap-8">
          {allSections.map((section, idx) => (
            <div
              key={idx}
              className={`finServ_agents_tab_block flex flex-col overflow-hidden rounded-[20px] bg-black lg:flex-row ${
