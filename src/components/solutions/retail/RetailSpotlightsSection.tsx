...
export function RetailSpotlightsSection() {
  const { spotlights } = retailPageData;
      ...
      <div className="container-default-sm">
        ...
        >
          {spotlights.items.map((item, i) => (
            <article
              key={i}
              className="flex min-w-[280px] max-w-[350px] flex-shrink-0 flex-col overflow-hidden rounded-[16px] bg-[#F5F5F9]"
            >
              ...
              <div className="flex flex-1 flex-col justify-between gap-[26px] p-[22px_18px]">
                ...
                {item.href ? (
                  ...
                  >
                    Read the story
                    <span className="flex h-[40px] w-[40px] items-center justify-center rounded-full bg-[#E4E7ED] transition-all duration-200 hover:bg-black">
